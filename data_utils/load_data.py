...
def load_images(class_name: str, train: bool, full_images: bool, n: int = -1, device: Optional[str] = None, switch_rgb: bool = True) -> list:
    ...
    images_paths = sorted(
        glob.glob(os.path.join(directory_path, "images/*.png"))
    )
    ...
    with tqdm(enumerate(images_paths), total=len(images_paths)) as pbar:
        for k, img_path in pbar:
            ...
            if switch_rgb: image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  
            transformed_data.append(image)
    return transformed_data


def load_gt_masks(class_name: str, train: bool, full_images: bool, n: int = -1, device: Optional[str] = None) -> list:
