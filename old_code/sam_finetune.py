    ...
    with torch.set_grad_enabled(keep_grad):
        ...
        binary_masks = torch.zeros(
            (
                ...
                original_image_size[1][0],
            ),
            device=device,
        )
        # upscaled_masks = sam_model.postprocess_masks(low_res_masks, input_size, original_image_size).to(device)
        for i in range(low_res_masks.shape[0]):
            upscaled_masks_one_image = sam_model.postprocess_masks(
                low_res_masks[i].unsqueeze(0),
                input_size,
                [original_image_size[0][i], original_image_size[1][i]],
            ).squeeze(0)
    ...
    for epoch in range(args.num_epochs):
        ...
        with tqdm(
            ...
            for k, data in pbar:
                ...
                binary_masks, gt_binary_masks = process_batch(
                    sam_model, data, keep_grad=True, device=device
                )
                ...
                if args.plot_every_batch > 0 and k % args.plot_every_batch == 0:
                    img_save_path = os.path.join(
                        plot_dir, f"train_epoch_{epoch}_batch_{k}.png"
                    )
                    show_predictions(
                        data[0], binary_masks, gt_binary_masks, img_save_path
                    )
                    ...
                    for i in range(len(gt_binary_masks)):
                        metrics = compute_all_metrics(
                            ground_truth=1 - gt_binary_masks[i],
                            prediction_binary=(
                                binary_masks[i] / torch.max(binary_masks[i])
                            )
                            < 0.5,
                            separate_images=args.use_full_images,
                        )
                ...
                # Compute loss
                loss = loss_fn(
                    binary_masks, gt_binary_masks.reshape(binary_masks.shape)
                )
                loss_of_batch += loss
                loss.backward()
        ...
        with tqdm(
            ...
            for k, data in pbar:
                set_description(pbar, description, k, frequency=1)
                binary_masks, gt_binary_masks = process_batch(
                    sam_model, data, keep_grad=False, device=device
                )

                # Plot the gt_binary_mask and binary_mask using matplotlib
                if args.plot_every_batch > 0 and k % args.plot_every_batch == 0:
                    img_save_path = os.path.join(
                        plot_dir, f"val_epoch_{epoch}_batch_{k}.png"
                    )
                    show_predictions(
                        data[0], binary_masks, gt_binary_masks, img_save_path
                    )
                ...
                for i in range(len(gt_binary_masks)):
                    metrics = compute_all_metrics(
                        ground_truth=1 - gt_binary_masks[i],
                        prediction_binary=(binary_masks[i] / torch.max(binary_masks[i]))
                        < 0.5,
                        separate_images=args.use_full_images,
                    )
                    list_metrics.append(metrics)
        ...
        if avg_metric > best_avg_metric:
            ...
            if args.save_model:
                torch.save(
                    sam_model.state_dict(), os.path.join(checkpoint_dir, "best.pth")
                )
...
def finetune_new(args):
    ...
    if args.use_full_images:
        training_data = FullImageDataset(
        ...
        training_data = CroppedImageDataset(
            class_name=args.class_name,
            train=True,
            n=args.n_train,
            device="cpu",
...
def parse_args():
    ...
    parser.add_argument(
        "--save_model", type=bool, default=True, help="Whether to save the model"
    )
    parser.add_argument(
        "--n_train", type=int, default=-1, help="Number of training images"
    )
    ...
    parser.add_argument(
        "--plot_every_batch", type=int, default=50, help="Plot predictions every"
    )
    parser.add_argument(
        "--use_full_images",
        type=bool,
        default=False,
        help="Whether to use full images or copped images",
    )
