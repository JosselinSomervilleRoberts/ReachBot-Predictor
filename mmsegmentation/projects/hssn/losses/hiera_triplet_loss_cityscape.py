...
def prepare_targets(targets):
    b, h, w = targets.shape
    targets_high = (
        torch.ones((b, h, w), dtype=targets.dtype, device=targets.device) * 255
    )
    indices_high = []
...
def losses_hiera(
    predictions, targets, targets_top, num_classes, indices_high, eps=1e-8
):
...
def losses_hiera_focal(
    predictions, targets, targets_top, num_classes, indices_high, eps=1e-8, gamma=2
):
    """Implementation of hiera loss.
    ...
    loss = (
        (
            (
                -targets[:, :num_classes, :, :]
                * torch.pow((1.0 - MCLA), gamma)
                * torch.log(MCLA + eps)
                - (1.0 - targets[:, :num_classes, :, :])
                * torch.pow(MCMA, gamma)
    ...
    loss += (
        (
            (
                -targets_top[:, :, :, :]
                * torch.pow((1.0 - MCLB), gamma)
                * torch.log(MCLB + eps)
                - (1.0 - targets_top[:, :, :, :])
                * torch.pow(MCMB, gamma)
                * torch.log(1.0 - MCMB + eps)
            )
            * valid_indices2
        ).sum()
        / num_valid2
...
class HieraTripletLossCityscape(nn.Module):
    ...
    def forward(
        self, step, embedding, cls_score_before, cls_score, label, weight=None, **kwargs
    ):
        targets, targets_top, indices_top = prepare_targets(label)

        loss = losses_hiera(
            cls_score, targets, targets_top, self.num_classes, indices_top
        )
        ...
        if torch.distributed.get_world_size() == torch.nonzero(
            class_counts, as_tuple=False
        ).size(0):
            factor = (
                1
                / 4
                * (1 + torch.cos(torch.tensor((step.item() - 80000) / 80000 * math.pi)))
                if step.item() < 80000
                else 0.5
            )
