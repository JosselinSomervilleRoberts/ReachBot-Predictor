...
def main():
    ...
    if not osp.exists(dataset_path):
        raise ValueError(
            "The dataset path does not exist. " "Please enter a correct dataset path."
        )
    if not osp.exists(osp.join(dataset_path, "img")) or not osp.exists(
        osp.join(dataset_path, "label")
    ):
        raise FileNotFoundError(
            "The dataset structure is incorrect. " "Please check your dataset."
        )
        ...
        for c in range(img_3d.shape[0]):
            ...
            img.save(
                osp.join(
                    save_path,
                    "img_dir/train",
                    "case" + idx.zfill(4) + "_slice" + str(c).zfill(3) + ".jpg",
                )
            )
            label.save(
                osp.join(
                    save_path,
                    "ann_dir/train",
                    "case" + idx.zfill(4) + "_slice" + str(c).zfill(3) + ".png",
                )
            )
